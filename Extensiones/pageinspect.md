...
# Ejemplo básico 
```sql
postgres@test# DROP TABLE tab1;
DROP TABLE
Time: 1.825 ms
postgres@test#
postgres@test#  create table tab1(id int, id2 int);
CREATE TABLE
Time: 3.686 ms
postgres@test# create index tab1_n1 on tab1(id);
CREATE INDEX
Time: 1.394 ms
postgres@test# insert into tab1 values(2,3);
INSERT 0 1
Time: 1.129 ms
postgres@test#
postgres@test#  select lp, t_xmin, t_xmax, t_ctid, t_infomask2,  cast(t_infomask2 as bit(16)), t_infomask, cast(t_infomask as bit(16)), t_data  from heap_page_items(get_raw_page('tab1',0));
+----+--------+--------+--------+-------------+------------------+------------+------------------+--------------------+
| lp | t_xmin | t_xmax | t_ctid | t_infomask2 |   t_infomask2    | t_infomask |    t_infomask    |       t_data       |
+----+--------+--------+--------+-------------+------------------+------------+------------------+--------------------+
|  1 |    804 |      0 | (0,1)  |           2 | 0000000000000010 |       2048 | 0000100000000000 | \x0200000003000000 |
+----+--------+--------+--------+-------------+------------------+------------+------------------+--------------------+
(1 row)

Time: 0.451 ms
postgres@test#   select * from bt_page_items('tab1_n1',1);
+------------+-------+---------+-------+------+-------------------------+------+-------+------+
| itemoffset | ctid  | itemlen | nulls | vars |          data           | dead | htid  | tids |
+------------+-------+---------+-------+------+-------------------------+------+-------+------+
|          1 | (0,1) |      16 | f     | f    | 02 00 00 00 00 00 00 00 | f    | (0,1) | NULL |
+------------+-------+---------+-------+------+-------------------------+------+-------+------+
(1 row)

Time: 0.331 ms
postgres@test#    UPDATE tab1 SET id = id * 10 WHERE id = 2;
UPDATE 1
Time: 1.495 ms
postgres@test#    select lp, t_xmin, t_xmax, t_ctid, t_infomask2,  cast(t_infomask2 as bit(16)), t_infomask, cast(t_infomask as bit(16)), t_data  from heap_page_items(get_raw_page('tab1',0));
+----+--------+--------+--------+-------------+------------------+------------+------------------+--------------------+
| lp | t_xmin | t_xmax | t_ctid | t_infomask2 |   t_infomask2    | t_infomask |    t_infomask    |       t_data       |
+----+--------+--------+--------+-------------+------------------+------------+------------------+--------------------+
|  1 |    804 |    805 | (0,2)  |           2 | 0000000000000010 |        256 | 0000000100000000 | \x0200000003000000 |
|  2 |    805 |      0 | (0,2)  |           2 | 0000000000000010 |      10240 | 0010100000000000 | \x1400000003000000 |
+----+--------+--------+--------+-------------+------------------+------------+------------------+--------------------+
(2 rows)
...
+------------+-------+---------+-------+------+-------------------------+------+-------+------+
|          1 | (0,1) |      16 | f     | f    | 02 00 00 00 00 00 00 00 | f    | (0,1) | NULL |
|          2 | (0,2) |      16 | f     | f    | 14 00 00 00 00 00 00 00 | f    | (0,2) | NULL |
+------------+-------+---------+-------+------+-------------------------+------+-------+------+
(2 rows)

Time: 0.394 ms
postgres@test# DELETE FROM tab1   WHERE id = 2;
DELETE 0
Time: 0.425 ms
postgres@test#
postgres@test#    select lp, t_xmin, t_xmax, t_ctid, t_infomask2,  cast(t_infomask2 as bit(16)), t_infomask, cast(t_infomask as bit(16)), t_data  from heap_page_items(get_raw_page('tab1',0));
...
| lp | t_xmin | t_xmax | t_ctid | t_infomask2 |   t_infomask2    | t_infomask |    t_infomask    |       t_data       |
+----+--------+--------+--------+-------------+------------------+------------+------------------+--------------------+
|  1 |    804 |    805 | (0,2)  |           2 | 0000000000000010 |       1280 | 0000010100000000 | \x0200000003000000 |
|  2 |    805 |      0 | (0,2)  |           2 | 0000000000000010 |      10240 | 0010100000000000 | \x1400000003000000 |
+----+--------+--------+--------+-------------+------------------+------------+------------------+--------------------+
(2 rows)
...
|  2 |    805 |      0 | (0,2)  |           2 | 0000000000000010 |      10496 | 0010100100000000 | \x1400000003000000 |
+----+--------+--------+--------+-------------+------------------+------------+------------------+--------------------+
(2 rows)
...
| itemoffset | ctid  | itemlen | nulls | vars |          data           | dead | htid  | tids |
+------------+-------+---------+-------+------+-------------------------+------+-------+------+
|          1 | (0,2) |      16 | f     | f    | 14 00 00 00 00 00 00 00 | f    | (0,2) | NULL |
+------------+-------+---------+-------+------+-------------------------+------+-------+------+
(1 row)

Time: 0.347 ms
postgres@test# vacuum FULL  tab1;
VACUUM
Time: 3.534 ms
postgres@test#
postgres@test#    select lp, t_xmin, t_xmax, t_ctid, t_infomask2,  cast(t_infomask2 as bit(16)), t_infomask, cast(t_infomask as bit(16)), t_data  from heap_page_items(get_raw_page('tab1',0));
...
| lp | t_xmin | t_xmax | t_ctid | t_infomask2 |   t_infomask2    | t_infomask |    t_infomask    |       t_data       |
+----+--------+--------+--------+-------------+------------------+------------+------------------+--------------------+
|  1 |    805 |      0 | (0,1)  |           2 | 0000000000000010 |      11008 | 0010101100000000 | \x1400000003000000 |
+----+--------+--------+--------+-------------+------------------+------------+------------------+--------------------+
(1 row)
...
| itemoffset | ctid  | itemlen | nulls | vars |          data           | dead | htid  | tids |
+------------+-------+---------+-------+------+-------------------------+------+-------+------+
|          1 | (0,1) |      16 | f     | f    | 14 00 00 00 00 00 00 00 | f    | (0,1) | NULL |
+------------+-------+---------+-------+------+-------------------------+------+-------+------+
(1 row)
