...
psql -p 5416       -c "COPY (select name ||' = ''' ||setting|| '''' from pg_settings ) TO '$dictory_backup/pg_settings-$fecha.csv'"
cp  /sysx/data16/postgresql.conf  $dictory_backup/postgresql.conf-RESP-$fecha
cp /sysx/data16/postgresql.auto.conf $dictory_backup/postgresql.auto.conf-RESP-$fecha
...
psql -p 5416     -c "select * from pg_catalog.pg_file_settings where error is not null; "

# Valida que el pg_hba.conf no tenga errores  
psql -p 5416     -c "select * from pg_hba_file_rules where error is not null; "
...
psql -p 5416     -xc "select *  from pg_settings where name = 'password_encryption'"
...
# ESTA QUERY TE MUESTRA LOS PARAMETROS QUE SE TIENEN QUE CONFIGURAR, TE INIDICA EL VALOR ACTUAL Y COMO DEBE DE ESTAR , TAMBIEN TE OTORGA EL SED DE LINUX PARA QUE REALICES LA MODIFICACION DE MANERA SEGURA 
 echo -e '\n\n' && psql -Xx -p 5412      -c " SELECT a.name as name_setting,setting as current_setting_incorrect, replace(coalesce(b.correct_setting,c.correct_setting),'/','\/') as correct_setting ,  short_desc,
  
 case when a.name != 'server_version' then  format('sed -i %s/%s[[:space:]]*=/ s/.*/%s = %s%s%s  #&/%s postgresql.conf ',chr(34) , a.name  , a.name , chr(39) ,replace(coalesce(b.correct_setting,c.correct_setting),'/','\/'),chr(39),chr(34) ) end as command_modify FROM pg_settings  as a 
...
4.- HACEMOS ENLACES SIMBOLICOS HACIA EL DISCO EN CASO DE MODIFICAR EL PARAMETRO log_directory
 -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	ln -s /pg_log/ /sysx/data16/pg_log 
...
psql -p 5416     -c "select pg_current_logfile();"
