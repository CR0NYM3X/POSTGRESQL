...
INNER JOIN INFORMATION_SCHEMA.KEY_COLUMN_USAGE t3 ON t0.CONSTRAINT_NAME = t3.CONSTRAINT_NAME
INNER JOIN INFORMATION_SCHEMA.KEY_COLUMN_USAGE t4 ON t0.UNIQUE_CONSTRAINT_NAME = t4.CONSTRAINT_NAME
ORDER BY 1, 2) TO '/tmp/receta/8_Detalle_Constraints.csv' WITH (FORMAT CSV, DELIMITER ',', HEADER);
...
/*
 ...
 psql -c "select t1.CONSTRAINT_SCHEMA AS Esquema, t0.CONSTRAINT_NAME as Nombre_Constraint, t1.TABLE_NAME as Tabla_FK,t3.COLUMN_NAME as Columna_FK,t2.TABLE_NAME as Tabla_PK,t4.COLUMN_NAME as Columna_PK from   INFORMATION_SCHEMA.REFERENTIAL_CONSTRAINTS t0 INNER JOIN INFORMATION_SCHEMA.TABLE_CONSTRAINTS t1 ON t0.CONSTRAINT_NAME = t1.CONSTRAINT_NAME INNER JOIN INFORMATION_SCHEMA.TABLE_CONSTRAINTS t2 ON t0.UNIQUE_CONSTRAINT_NAME =  t2.CONSTRAINT_NAME INNER JOIN INFORMATION_SCHEMA.KEY_COLUMN_USAGE t3 ON t0.CONSTRAINT_NAME = t3.CONSTRAINT_NAME INNER JOIN INFORMATION_SCHEMA.KEY_COLUMN_USAGE t4 ON t0.UNIQUE_CONSTRAINT_NAME = t4.CONSTRAINT_NAME ORDER BY 1, 2" | grep -v "+" | tr "|" "," > /tmp/receta/8_Detalle_Constraints.csv
