...
COPY (select 'REINDEX INDEX ' || indexrelname || ';' as Script_Fragmentacion, t1.relname as Tabla, t1.reltuples AS Registros,
case when idx_blks_hit = 0 then 0 else ((idx_blks_hit - idx_blks_read) / cast((idx_blks_hit) as float))*100 end as Porcentaje_Fragmentacion
from pg_statio_all_indexes t0
inner join pg_class t1 on t0.relname = t1.relname
LEFT JOIN pg_namespace t2 ON t2.oid = t1.relnamespace
where schemaname = 'public'
order by 3) TO '/tmp/receta/5_Fragmentacion_Indices.csv' WITH (FORMAT CSV, DELIMITER ',', HEADER);



/*

 psql -c "select 'REINDEX INDEX ' || indexrelname || ';' as Script_Fragmentacion, t1.relname as Tabla, t1.reltuples AS Registros,case when idx_blks_hit = 0 then 0 else ((idx_blks_hit - idx_blks_read) / cast((idx_blks_hit) as float))*100 end as Porcentaje_Fragmentacion from pg_statio_all_indexes t0 inner join pg_class t1 on t0.relname = t1.relname  LEFT JOIN pg_namespace t2 ON t2.oid = t1.relnamespace where schemaname = 'public' order by 3" | grep -v "+" | tr "|" "," > /tmp/receta/5_Fragmentacion_Indices.csv
